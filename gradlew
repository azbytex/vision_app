...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # temporary variables.  (e.g. if 3 times, then the variables arg1,
        # arg2, arg3, are set in a loop and then used as "$arg1" "$arg2" "$arg3")
        # shellcheck disable=SC2086 disable=SC2043
        set -- "$@" "$arg"
        shift
    done
fi
...
# post-process each arg (bash "read" is not POSIX).  This is
# temporary until Gradle builds on JVM that is no longer affected
# by this issue.  See Gradle#5245.
#
...
# Run with "xargs" to strip any newline added by the "eval" command above.
